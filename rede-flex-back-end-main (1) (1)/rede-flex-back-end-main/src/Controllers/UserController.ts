...
class UserController {
    ...
    public async registerUsers(req: Request, res: Response) {
        return res.status(500).json({ message: `Função desabilitada.` })
        try {
    ...
    public async login(req: Request, res: Response) {
        try {
            ...
            return res.status(200).json({ message: "Login efetuado com sucesso!", acesso: token, use_name: existingEmail.use_name, use_level: existingEmail.use_level });
        } catch (error) {
    ...
    public async sendEmail(req: Request, res: Response) {
        try {
            ...
            const mailOptions = {
                ...
                <a href="https://redeflexbi.com.br/password_recovery?use_token=${use_token}&use_email=${use_email}">Recuperar Senha</a>
              `,
            };
